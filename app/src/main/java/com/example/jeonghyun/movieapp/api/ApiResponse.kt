...
sealed class ApiResponse<out T> {
    class Success<T>(response: Response<T>): ApiResponse<T>() {
        ...
        override fun toString(): String = "[ApiReponse.Success]: $data"
    }

    sealed class Failure<T> {
        class Error<T>(response: Response<T>): ApiResponse<T>() {
            val responseBody: ResponseBody? = response.errorBody()
            val code: Int = response.code()
            override fun toString(): String = "[Apireponse.Failure $code]: $responseBody"
        }

        class Exception<T>(exception: Throwable): ApiResponse<T>() {
